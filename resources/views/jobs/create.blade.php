<x-layout :title="__('messages.jobs.create.title')">
    <x-slot:heading>{{ __('messages.jobs.create.heading') }}</x-slot:heading>
    ...
    <form method="POST" action="{{ route('jobs.store') }}">
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6">
            <a href="{{ route('jobs.index') }}" class="text-sm/6 font-semibold text-gray-900">{{ __('messages.jobs.create.cancel') }}</a>
            <button type="submit" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus:outline-indigo-600">{{ __('messages.jobs.create.create_button') }}</button>
        </div>
    </form>
</x-layout>
